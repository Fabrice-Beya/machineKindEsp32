...
class CDataLogger
{
  public:
    ...
    {
        //TODO: Extend to send through GPRS
        #if ENABLE_DATA_CACHE
    ...
    void logData(uint16_t pid, char *value)
    {
        char buf[64];
        //Het friendly PID Name for logging
        byte n = translatePIDName(pid, buf);
        byte len = sprintf(buf + n, " ", value) + n;
        dispatch(buf, len);
        record(buf, len);
    }
